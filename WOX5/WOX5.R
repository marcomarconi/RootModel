...
res <- group_by(df %>% filter(Induction == 1), Type, WOX5) %>% mutate(WOX5 = factor(case_when(between(WOX5, 0, 0.2) ~ "Low", between(WOX5, 0.4, 0.6) ~ "Medium", between(WOX5, 0.8, 1.0) ~ "High",TRUE~NA))) %>% summarise(Mean=mean(Divisions), SD=sd(Divisions)/sqrt(n()))
res %>% mutate(WOX5=factor(WOX5, levels=c("Low", "Medium", "High")))  %>%  ggplot(aes(fill=WOX5)) +
  geom_bar(aes(Type, Mean), stat="identity", position = position_dodge(width=0.9)) + 
  geom_errorbar(aes(Type, ymin=Mean-SD, ymax=Mean+SD), width=0.25, position = position_dodge(width=0.9)) +
  theme(axis.title.x = element_blank(), text=element_text(size = 24), legend.background = element_rect(colour = "black")) + 
  ylab("QC Division (%)") + ylim(c(0, 1)) + labs(fill='WOX5 prod') 
